...
import com.sorter.component.SorterBase;
import com.sorter.exception.IterableSorterException;
...
public class IteratingSorter<T> extends SorterBase<T> implements Closeable
{
	...
	public IteratingSorter(SortConfiguration config, DataReaderFactory<T> readerFactory, DataWriterFactory<T> writerFactory, Comparator<T> comparator) {
	...
	public IteratingSorter(SortConfiguration config) {
		super(config);
	}
	 ...
	 * The returned Iterator will throw IterableSorterException} if any IOException is encountered during calls of Iterator#next().
	...
	public Iterator<T> sort(DataReader<T> inputReader) throws IOException {
		...
		try {
			...
			/* In case all entries might fit in
